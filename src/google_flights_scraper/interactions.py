...
def enter_departure_airport(wait, airport_code):
    """Enter departure airport code into input field.

    Args:
        wait: WebDriverWait instance
        airport_code: IATA code for departure airport
    ...
    from_input.click()
    time.sleep(1)
    from_popup = wait.until(EC.presence_of_all_elements_located((
        By.XPATH, "//input[contains(@aria-label,'Where else?')]",
    )))[1]
    from_popup.clear()
...
def enter_arrival_airport(wait, airport_code):
    """Enter arrival airport code into input field.

    Args:
        wait: WebDriverWait instance
        airport_code: IATA code for arrival airport
    ...
    to_input.click()
    to_popup = wait.until(EC.presence_of_all_elements_located((
...
def enter_airports(wait, airport_code_from, airport_code_to):
    """Enter both departure and arrival airport codes.

    Args:
        wait: WebDriverWait instance
        airport_code_from: IATA code for departure airport
        airport_code_to: IATA code for arrival airport
    """
    enter_departure_airport(wait, airport_code_from)
    enter_arrival_airport(wait, airport_code_to)


def enter_departure_date(wait, date_from):
    """Enter departure date into input field.

    Args:
        wait: WebDriverWait instance
        date_from: Departure date in MM/DD/YYYY format

    Raises:
        Exception: If elements cannot be found or interacted with
    """
    try:
        departure_input = wait.until(EC.presence_of_element_located(
    ...
    try:
        departure_popup = wait.until(EC.presence_of_all_elements_located(
            (By.XPATH, "//input[@aria-label='Departure']"),
        ))[1]
    except TimeoutException as e:
        raise Exception("Error entering departure date:") from e
...
def enter_return_date(wait, date_to):
    """Enter return date into input field.

    Args:
        wait: WebDriverWait instance
        date_to: Return date in MM/DD/YYYY format

    Raises:
        Exception: If elements cannot be found or interacted with
    """
    try:
        return_popup = wait.until(EC.presence_of_all_elements_located(
...
def enter_dates(wait, date_from, date_to):
...
def select_seat_class(wait, seat_class, is_domestic_us):
    """Select seat class from dropdown.

    Args:
        wait: WebDriverWait instance
        seat_class: Seat class to select
    ...
    try:
        class_dropdown = wait.until(EC.presence_of_element_located(
            (By.XPATH, "//div[@role='combobox' and .//span[@aria-label='Change seating class.']]"),
        ))
    except TimeoutException as e:
    ...
    try:
        class_options = wait.until(EC.presence_of_all_elements_located(
...
def wait_until_class_stable(driver, locator, stable_duration=1.0, timeout=10):
    """Wait until an element's class attribute stops changing.

    Args:
        driver: Selenium WebDriver instance
        locator: Tuple of (By.*, 'selector') e.g., (By.ID, 'my-div')
        stable_duration: How long (in seconds) the class must remain unchanged
        timeout: Maximum time to wait before raising TimeoutException
...
def find_and_select_best_flight(wait, driver, timeout):
    ...
    try:
        best_flight = wait.until(EC.presence_of_element_located(
